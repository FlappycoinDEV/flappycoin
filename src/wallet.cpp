...
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl) const
{
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            {
                if (!(pcoin->IsSpent(i)) && IsMine(pcoin->vout[i]) && !IsLockedCoin((*it).first, i) && pcoin->vout[i].nValue >= nMinimumInputValue &&
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew, uint256& blockFromHash)
{
        ...
        for (unsigned int n=0; n<min(nSearchInterval,(int64)nMaxStakeSearchInterval) && !fKernelFound; n++)
        {
            ...
            if (CheckStakeKernelHashScrypt(nBits, (unsigned int)pcoin.first->GetTxTime(), pcoin.first->hashBlock, TxFirst, prevoutStake, txNew.nTime, hashProofOfStake))
            {
               // Found a kernel
                ...
                blockFromHash = pcoin.first->hashBlock;
                if (fDebug && GetBoolArg("-printcoinstake"))
...
bool CWallet::SelectCoins(int64 nTargetValue, unsigned int nSpendTime, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet, const CCoinControl* coinControl) const
{
    ...
    BOOST_FOREACH(COutput output, vCoins)
    {
        ...
        if ( (output.tx->GetTxTime() + nStakeMinAge < GetTime())  && (output.tx->vout[0].nValue + output.tx->vout[1].nValue ) > 1 )
        {
            setCoinsRet.insert(coin.second);
            nValueRet += coin.first;
            return true;
